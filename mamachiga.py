...
#configurations
VOSK_MODEL_PATH = os.path.expanduser("vosk-model-small-en-us-0.15") # OR ~models/vosk/vosk-model-small-en-us-0.15 for anonto
CAMERA_INDEX = 0
FRAME_WIDTH = 460
FRAME_HEIGHT = 460
FOCAL_LENGTH = 700.0
...
# api key
load_dotenv()
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "you_api_key")  
...
jarvis_processing = threading.Event()  
alpha_mode_queue = queue.Queue()  
...
def query_gemini_text_only(question):
    ...
    try:
        response = gemini_model.generate_content(f"Answer this question in exactly 1 sentence, dont speak too much : {question}")
        return response.text
    except Exception as e:
        print(f"Gemini API error: {e}")
        return f"Sorry, I encountered an error: {str(e)}"
...
def handle_jarvis_command(command_text, image_bgr):
    ...
    try:
        ...
        if any(cmd in command_lower for cmd in JARVIS_COMMANDS["read"]):
            prompt = """Extract and read all visible text from this image. Only tell the text that you can extract. no explanation needed.
        ...
        # Product 
            ...
            prompt = """Identify product in maximum 2 sentences: name, brand, and most important detail only. Dont explain it further"""
            response = query_gemini_with_image(image_bgr, prompt)
            speak_text(response, force=True, priority=True, mode="jarvis")
        
        # Currency
        elif any(cmd in command_lower for cmd in JARVIS_COMMANDS["currency"]):
            prompt = """Identify currency : denomination and total value only. Dont further explain anything"""
            response = query_gemini_with_image(image_bgr, prompt)
            speak_text(response, force=True, priority=True, mode="jarvis")
        ...
        # Smart Q&A
            ...
            # General description
            prompt = """Describe what you see in maximum 1 sentences: main objects only. 
            Dont explain the objects too much. 
            Explain the surrounding ONLY if there are not many objects in front
            If you are unsure of what you are seeing dont answer confidently"""
            response = query_gemini_with_image(image_bgr, prompt)
            speak_text(response, force=True, priority=True, mode="jarvis")
    finally:
...
def jarvis_worker():
    ...
    while True:
        try:
            command, image = jarvis_command_queue.get(timeout=1)
            if command is None:
                break
...
def alpha_mode_worker():
    ...
    while True:
        try:
            question = alpha_mode_queue.get(timeout=1)
            if question is None:
                break
...
def audio_callback(indata, frames, time_info, status):
    try:
        ...
        return
    # conv audio to numpy and then use Root Mean Square
        ...
        while True:
            ...
            if rec.AcceptWaveform(data):
                ...
                if text:
                    ...
                    # Priority commands for mode switching
